...
![](lab7/water.png)
#### Group Members:
My partner, Matthew, worked on the part 1 portion for this lab and created a scene in Unity with an island, which he used basic shapes with vertex shader on them to make them look rigid like a stone island. He also added a yellowish light flowing out from the center of the island which imitated the last scene from a favorite film of his, which was pretty amusing. Lastly, for the water's waves, he chose to use a height map on it in order to make it look more realistic instead of just using a basic sine wave motion to its vertex shader, and that was pretty impressive.
